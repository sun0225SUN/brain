...
// node_modules/.pnpm/mermaid@10.1.0/node_modules/mermaid/dist/erDiagram-215b0341.js
var ot = function() {
  var t = function(v, n, s, d) {
    for (s = s || {}, d = v.length; d--; s[v[d]] = n)
      ;
    return s;
  }, r = [1, 2], e = [1, 5], a = [6, 9, 11, 23, 25, 27, 29, 30, 31, 51], c = [1, 17], p = [1, 18], f = [1, 19], l = [1, 20], h = [1, 21], _ = [1, 22], g = [1, 25], E = [1, 30], O = [1, 31], y = [1, 32], M = [1, 33], B = [6, 9, 11, 15, 20, 23, 25, 27, 29, 30, 31, 44, 45, 46, 47, 51], V = [1, 45], z = [30, 31, 48, 49], L = [4, 6, 9, 11, 23, 25, 27, 29, 30, 31, 51], R = [44, 45, 46, 47], N = [22, 37], T = [1, 65], x = [1, 64], S2 = [22, 37, 39, 41], C = {
    trace: function() {
    },
    yy: {},
...
Expecting ` + $.join(", ") + ", got '" + (this.terminals_[I] || I) + "'" : nt = "Parse error on line " + (q + 1) + ": Unexpected " + (I == ht ? "end of input" : "'" + (this.terminals_[I] || I) + "'"), this.parseError(nt, {
    ...
    var v = {
      ...
      setInput: function(n, s) {
        return this.yy = s || this.yy || {}, this._input = n, this._more = this._backtrack = this.done = false, this.yylineno = this.yyleng = 0, this.yytext = this.matched = this.match = "", this.conditionStack = ["INITIAL"], this.yylloc = {
          first_line: 1,
          first_column: 0,
          last_line: 1,
          last_column: 0
...
` + s + "^";
      ...
      next: function() {
        ...
        for (var m = this._currentRules(), i = 0; i < m.length; i++)
          if (d = this._input.match(this.rules[m[i]]), d && (!s || d[0].length > s[0].length)) {
            if (s = d, u = i, this.options.backtrack_lexer) {
              if (n = this.test_match(d, m[i]), n !== false)
                return n;
              if (this._backtrack) {
                s = false;
                continue;
        ...
        return s ? (n = this.test_match(s, m[u]), n !== false ? n : false) : this._input === "" ? this.EOF : this.parseError("Lexical error on line " + (this.yylineno + 1) + `. Unrecognized text.
` + this.showPosition(), {
          text: "",
          token: null,
          line: this.yylineno
...
var Zt2 = function(t, r, e) {
  vs.parseDirective(this, t, r, e);
};
var yt = function(t) {
...
var Pt = function(t, r) {
  ...
  for (a = r.length - 1; a >= 0; a--)
    e.attributes.push(r[a]), et.debug("Added attribute ", r[a].attributeName);
};
var Wt = function(t, r, e, a) {
  let c = {
    entityA: t,
    roleA: r,
    entityB: e,
...
function Xt(t, r = 0) {
  return (A2[t[r + 0]] + A2[t[r + 1]] + A2[t[r + 2]] + A2[t[r + 3]] + "-" + A2[t[r + 4]] + A2[t[r + 5]] + "-" + A2[t[r + 6]] + A2[t[r + 7]] + "-" + A2[t[r + 8]] + A2[t[r + 9]] + "-" + A2[t[r + 10]] + A2[t[r + 11]] + A2[t[r + 12]] + A2[t[r + 13]] + A2[t[r + 14]] + A2[t[r + 15]]).toLowerCase();
}
function Qt(t) {
  if (!Ht(t))
...
var se = (t, r, e) => {
  ...
  let h = false, _ = false, g = 0, E = 0, O = 0, y = 0, M = f.height + a * 2, B = 1;
  e.forEach((R) => {
    ...
    let T = 0;
    const x = Il(R.attributeType), S2 = t.append("text").classed("er entityLabel", true).attr("id", `${N}-type`).attr("x", 0).attr("y", 0).style("dominant-baseline", "middle").style("text-anchor", "left").style("font-family", po().fontFamily).style("font-size", p + "px").text(x), C = t.append("text").classed("er entityLabel", true).attr("id", `${N}-name`).attr("x", 0).attr("y", 0).style("dominant-baseline", "middle").style("text-anchor", "left").style("font-family", po().fontFamily).style("font-size", p + "px").text(R.attributeName), D = {};
    D.tn = S2, D.nn = C;
    const Y = S2.node().getBBox(), v = C.node().getBBox();
    if (g = Math.max(g, Y.width), E = Math.max(E, v.width), T = Math.max(Y.height, v.height), h) {
      const n = R.attributeKeyTypeList !== void 0 ? R.attributeKeyTypeList.join(",") : "", s = t.append("text").classed("er entityLabel", true).attr("id", `${N}-key`).attr("x", 0).attr("y", 0).style("dominant-baseline", "middle").style("text-anchor", "left").style("font-family", po().fontFamily).style("font-size", p + "px").text(n);
      D.kn = s;
      const d = s.node().getBBox();
      O = Math.max(O, d.width), T = Math.max(T, d.height);
    }
    if (_) {
      const n = t.append("text").classed("er entityLabel", true).attr("id", `${N}-comment`).attr("x", 0).attr("y", 0).style("dominant-baseline", "middle").style("text-anchor", "left").style("font-family", po().fontFamily).style("font-size", p + "px").text(R.attributeComment || "");
      D.cn = n;
  ...
  if (e.length > 0) {
    ...
    l.forEach((x) => {
      const S2 = N + a + x.height / 2;
      x.tn.attr("transform", "translate(" + c + "," + S2 + ")");
      const C = t.insert("rect", "#" + x.tn.node().id).classed(`er ${T}`, true).attr("x", 0).attr("y", N).attr("width", g + c * 2 + R).attr("height", x.height + a * 2), D = parseFloat(C.attr("x")) + parseFloat(C.attr("width"));
      x.nn.attr(
        "transform",
        "translate(" + (D + c) + "," + S2 + ")"
      );
      const Y = t.insert("rect", "#" + x.nn.node().id).classed(`er ${T}`, true).attr("x", D).attr("y", N).attr("width", E + c * 2 + R).attr("height", x.height + a * 2);
      ...
      if (h) {
        ...
        const n = t.insert("rect", "#" + x.kn.node().id).classed(`er ${T}`, true).attr("x", v).attr("y", N).attr("width", O + c * 2 + R).attr("height", x.height + a * 2);
...
var oe = function(t, r, e) {
  ...
  return a.forEach(function(p) {
    ...
    c = c === void 0 ? f : c;
    const h = "text-" + f, _ = l.append("text").classed("er entityLabel", true).attr("id", h).attr("x", 0).attr("y", 0).style("dominant-baseline", "middle").style("text-anchor", "middle").style("font-family", po().fontFamily).style("font-size", k.fontSize + "px").text(p), { width: g, height: E } = se(
      l,
      _,
      r[p].attributes
    ), y = l.insert("rect", "#" + h).classed("er entityBox", true).attr("x", 0).attr("y", 0).attr("width", g).attr("height", E).node().getBBox();
    e.setNode(f, {
      width: y.width,
      height: y.height,
      shape: "rect",
...
var he = function(t, r, e, a, c) {
  ...
  const p = e.edge(
    ...
    return M.y;
  }).curve(We), l = t.insert("path", "#" + a).classed("er relationshipLine", true).attr("d", f(p.points)).style("stroke", k.stroke).style("fill", "none");
  ...
  const _ = l.node().getTotalLength(), g = l.node().getPointAtLength(_ * 0.5), E = "rel" + ut, y = t.append("text").classed("er relationshipLabel", true).attr("id", E).attr("x", g.x).attr("y", g.y).style("text-anchor", "middle").style("dominant-baseline", "middle").style("font-family", po().fontFamily).style("font-size", k.fontSize + "px").text(r.roleA).node().getBBox();
  t.insert("rect", "#" + E).classed("er relationshipLabelBox", true).attr("x", g.x - y.width / 2).attr("y", g.y - y.height / 2).attr("width", y.width).attr("height", y.height);
};
var de = function(t, r, e, a) {
  k = po().er, et.info("Drawing ER diagram");
  const c = po().securityLevel;
  let p;
  c === "sandbox" && (p = yh("#i" + r));
  const l = (c === "sandbox" ? yh(p.nodes()[0].contentDocument.body) : yh("body")).select(`[id='${r}']`);
  W.insertMarkers(l, k);
  let h;
  h = new A({
    multigraph: true,
  ...
  zs(h), ce(l, h), g.forEach(function(B) {
    he(l, B, h, _, a);
  });
  const E = k.diagramPadding;
  _s.insertTitle(l, "entityTitleText", k.titleTopMargin, a.db.getDiagramTitle());
  const O = l.node().getBBox(), y = O.width + E * 2, M = O.height + E * 2;
  Zt(l, M, y, k.useMaxWidth), l.attr("viewBox", `${O.x - E} ${O.y - E} ${y} ${M}`);
};
...
//# sourceMappingURL=erDiagram-215b0341-4DTEJRYR.js.map
